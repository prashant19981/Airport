...
public class carrierControl : MonoBehaviour {
	...
	void pathMaker(Vector2 point){
		...
		try{
			...
			if(shopManagerlevel4.isILSRunway1On){
	...
	void OnTriggerEnter2D(Collider2D other){
		try{
			if (other.tag == "Airplane" ) {
				if (!hasLanded && !other.gameObject.GetComponent<carrierControl> ().hasLanded || !hasLanded && !other.gameObject.GetComponent<HelicopterControl> ().hasLanded ) {
					if (isOnScreen) {
	...
	void OnTriggerExit2D(Collider2D other){
		if (other.tag == "Airplane" || other.tag == "Helicopter") {
			if (isOnScreen) {
