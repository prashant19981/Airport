...
//	Vector2[] navPoints = new Vector2[12];
	...
	private String temp;
	private String prevDir = "Straight";
	private readonly String[] planeYaw = {"Left","Right","Straight"};
	Vector3 scale;
	...
	void Start(){
		  Starter ();
		//StartCoroutine(planeSpinner());
		//planeAnimation = GetComponent<Animation>();
		lockDeterminer(); 
...
//		}
	...
	void Update(){
		...
		if (Input.GetMouseButton (0) && !takeTaxiControl && Input.touchCount < 2) {
			...
			mouse = new Vector3 (Input.mousePosition.x, Input.mousePosition.y, 10f);
			mousePos = new Vector2 (Camera.main.ScreenToWorldPoint (mouse).x, Camera.main.ScreenToWorldPoint (mouse).y);
			if (Vector2.Distance (mousePos, new Vector2 (transform.position.x, transform.position.y)) < 0.3f && !canControl && isSelected) {
				canControl = true;
				colorNormalizer ();
				try{
				points.Clear ();
		...
		if (points != null && points.Count!=0) {
			...
			//print ("current "+ transform.rotation + " target "+ targetAngle +" target "+ (targetAngle.z - transform.rotation.z) );
			//print("angle " + angle); 
			prevAngle = angle;
...
//				
	...
	void respawner(){
		gameObject.GetComponent<insidePlane>().setIsActive(false);
		            ...
		            Vector3 correct = new Vector3(0, 0, 3f);
	...
	//IEnumerator planeSpinner(){
	/*print ("Angle:" + angle + " Prev:" + prevAngle);
	...
	//print ("HERE");
	/*temp = GetRotateDirection (transform.rotation, targetAngle);
	if (temp == "Left") {
		//print ("Left");
		if (prevDir == "Right")
		{
			planeAnimation["left"].speed = -1f;
			planeAnimation.Play("left");
			GetComponent<Animator>().SetTrigger("leftToRight");
		}
		else
		{
			planeAnimation["left"].speed = 1f;
	...
	*/
	...
	String GetRotateDirection(Quaternion from, Quaternion to)
	{
		float fromY = from.eulerAngles.z;
		float toY  = to.eulerAngles.z;
		float clockWise = 0f;
		float counterClockWise = 0f;
		//print ("FromY:" + fromY + " ToY:" + toY);
		if (fromY < toY)
		{
			//return false;
			clockWise = toY-fromY;
		...
		//true if right turn
		//print ("Clockwise:" + clockWise + "Counter:" + counterClockWise);
		//If value of counter or clockwise is less than 2 then assume plane is going straight
		if (clockWise < 2f || counterClockWise < 2f) {
			return planeYaw [2];
